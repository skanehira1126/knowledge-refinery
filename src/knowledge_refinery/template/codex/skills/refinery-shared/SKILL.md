...
2. `knowledge-refinery review search` で review キューを確認する。
3. 必要に応じて `knowledge-refinery review refresh ...` で review を最新の flow に追従させる。
4. review から stock へ上げる前に、`shared/stock` を `title`, `summary`, `knowledge_id`, `knowledge_type`, `tags` で確認し、既存 stock を更新するか新規作成するか判断する。
5. review 済みの候補を `knowledge-refinery review promote ...` で `shared/stock` へコピーする。`knowledge_id` が type 違いで曖昧な場合は `--knowledge-type reference|constructive` を併用する。
6. 不採用の候補は `knowledge-refinery review reject ...` で review キューから外す。`knowledge_id` が type 違いで曖昧な場合は `--knowledge-type reference|constructive` を併用する。
...
- `shared/stock` を手動で新規作成または front matter 更新するときは、手書き YAML ではなく `knowledge-refinery knowledge upsert --scope stock ...` を優先する。
...
- `shared/stock` を手動で新規作成または front matter 更新した後は `knowledge-refinery knowledge search --scope stock` で読み取り確認する。
