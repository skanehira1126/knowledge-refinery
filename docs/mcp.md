...
| `refinery_delete_experience` | experience削除のdry-runと確認済み実行 | `project_path` |
...
| `refinery_delete_memory` | memory削除のdry-runと確認済み実行 | `project_path` |
...
memory検索ではどのproject scopeでもshared memoryを対象にします。statusを省略したmemory検索は
`active`だけを返します。
...
Experienceの`statuses`は`completed`、`inconclusive`、`abandoned`、`superseded`、memoryの
`statuses`は`active`、`superseded`、`retracted`、`confidences`は
...
Memoryを置換するときはactiveな後継を同じscopeへ先に保存し、旧memoryを`status: superseded`、
`superseded_by: <後継ID>`へ更新します。後継なしの撤回は`status: retracted`を使います。

Delete toolは`confirm`の既定値がfalseで、revisionと参照影響を返すだけです。`can_delete: true`を
確認し、利用者の明示承認を得た後、同じ`expected_updated_at`と`confirm: true`で再実行します。
参照元またはvalidation不能なknowledge文書が一つでもあれば削除せず、blockerを返します。
