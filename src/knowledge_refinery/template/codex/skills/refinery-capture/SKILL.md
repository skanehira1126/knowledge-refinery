...
2. 同じ topic の raw ファイルがあれば追記し、なければ `knowledge-refinery skills upsert-knowledge --scope raw ...` で新規 Markdown ファイルを追加する。
...
- `knowledge-refinery skills upsert-knowledge --scope raw --session-id "<session_id>" --file "<file>.md" --title "<title>" --description "<description>" --body-file "<body_file>"`
...
- 新規作成または front matter 更新後は `knowledge-refinery skills search knowledge --session-id "<session_id>" --scope raw` で読み取り確認する。
