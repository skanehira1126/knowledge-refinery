...
3. `knowledge-refinery skills upsert-knowledge --scope flow ...` で `flow/` の Markdown 知識ファイルを追加または更新し、少なくとも `title`, `description`, `summary` を埋める。`reference` / `constructive` を区別できる場合は `knowledge_type` も付ける。
...
- `knowledge-refinery skills upsert-knowledge --scope flow --session-id "<session_id>" --file "<file>.md" --title "<title>" --description "<description>" --summary "<summary>" --knowledge-type reference|constructive --body-file "<body_file>"`
...
- front matter を新規作成または更新するときは、手書き YAML ではなく `knowledge-refinery skills upsert-knowledge` を優先する。
- CLI を使えず手動で front matter を直す場合だけ、文字列値を原則ダブルクォートし、特に `title`, `description`, `summary` にバッククォート、コロン、角括弧、改行を含む場合は必ず引用する。
- 新規作成または front matter 更新後は `knowledge-refinery skills search knowledge --session-id "<session_id>" --scope flow` で読み取り確認する。
