...
- `knowledge-refinery knowledge search` や `knowledge-refinery review search` が front matter エラーで落ちたとき
- `knowledge-refinery session search` が `meta.yaml` の不正で読めないとき
- `knowledge-refinery review prepare`, `knowledge-refinery review refresh`, `knowledge-refinery review promote` 実行前に knowledge file の形式を整えたいとき
...
4. `title`, `description`, `summary`, `knowledge_id`, `knowledge_type`, `source_sessions`, `derived_from` などの必須項目を運用レイヤーに応じて確認する。
5. 配列・`null`・真偽値・数値を文字列化せず、既存の意味を保ったまま整える。
6. `knowledge-refinery knowledge search`, `knowledge-refinery review search`, `knowledge-refinery session search` など該当 CLI で再検証する。

## Preferred commands

- `knowledge-refinery knowledge search --root .refinery`
- `knowledge-refinery review search --root .refinery`
- `knowledge-refinery session search --root .refinery`
...
- `knowledge_type` を補う場合は `reference` または `constructive` のどちらかに限定する。
...
- `knowledge_type` が既にある knowledge は削除せず保ち、値は `reference` / `constructive` のみを使う。
