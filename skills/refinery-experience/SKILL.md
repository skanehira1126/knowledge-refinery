...
Do not physically delete experience as routine cleanup. Prefer `superseded` for a conclusion replaced by a later saved experience. When deletion is explicitly requested, exact-get the target, call `refinery_delete_experience` without `confirm`, present its revision, structured references, and validation errors, and obtain explicit user confirmation. Retry with the same `expected_updated_at` and `confirm: true` only when `can_delete` is true. Never remove or bypass a blocking reference implicitly.
