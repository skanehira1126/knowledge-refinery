...
def resolve_agents_path(target: Path) -> Path:
    ...
    if resolved.name == "AGENTS.md":
        return resolved
    return resolved / "AGENTS.md"


def load_agents_snippet(lang: str) -> str:
    if lang not in LANG_CHOICES:
        raise ValueError(f"Unsupported language: {lang}")
    return files("knowledge_refinery.data").joinpath(f"agents.{lang}.md").read_text(encoding="utf-8").rstrip()
...
def apply_agents_md(target: Path, lang: str) -> Path:
