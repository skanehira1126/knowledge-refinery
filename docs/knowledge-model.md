...
| 確からしさ | `confidence`, `updated_at` |
| 根拠 | `source_experiences` |

Project memoryは現在projectのexperienceを最低1件参照します。Shared memoryは
...
| status | experienceの`statuses` / `--status` | 完全一致 |
...
検索の既定scopeは現在projectです。Memory検索にはshared memoryも常に含まれます。
他projectのexperienceまたはproject memoryへ広げる場合は、目的を確認して
`all_projects: true`または`--all-projects`を指定します。
