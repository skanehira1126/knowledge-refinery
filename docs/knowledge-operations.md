...
| project memoryの作成・更新 | Codex / 作業者 | 原則2件以上の反復・相補的根拠と適用範囲。1件は利用者の明示依頼が必要 |
| shared memoryの新規作成・昇格 | Codexが候補を提示、利用者が承認 | 2 project以上の独立した根拠、scope、限界、反例、confidence |
...
「この結果によって、将来のagentが何かを選ぶ、避ける、検証する、診断する方法が変わるか」を
record gateにします。答えがyesなら記録し、単なる完了報告、進捗log、明白なtypo修正、
新しい根拠・条件・反例を加えない反復は記録しません。
...
statusとconfidenceは[ナレッジモデルと検索](knowledge-model.md#status-confidence)の
決定表に従います。評価可能な否定結果は`completed`であり、失敗したという理由だけで
`inconclusive`や`abandoned`にはしません。

作成前に内容を表す安定したlowercase slugを`experience_id`として決めます。createのresponseを
受け取れず結果が不明な場合は、同じcallをすぐ再実行せず、そのIDをexact getし、必要ならID検索します。
存在しないことを確認した場合だけcreateをretryします。

既存experienceを更新するときは、直前に取得した `updated_at` を
`expected_updated_at` として渡します。stale revisionが拒否された場合は、最新内容を読み直し、
競合する変更を統合してから再実行します。
更新時のoptional fieldは、省略すると現在値を保持し、空listを明示した場合だけlistをclearします。
confidenceをclearするときは`clear_confidence: true`を明示します。

### 2. Project memoryへ抽出する

同じproject内で繰り返し利用できる判断原則をproject memoryへ抽出します。一度だけ観察した事実や、
適用条件が不明な知見はexperienceに留めます。原則として、同じ判断を反復して支持する、または
異なる条件から相補的に検証する2件以上のexperienceを根拠にします。
...
利用者が1件のexperienceからmemoryを作るよう明示依頼した場合だけ例外を認めます。その場合は
scopeを確認済み条件へ狭め、未反復であることと追加検証事項を本文へ書き、confidenceは`low`または
`medium`とし、`high`にはしません。

### 3. Shared memoryへ昇格する

shared memoryは、異なる2 project以上のexperienceが同じ原則を支持するときだけ候補にします。
sourceはすべて `project-id/experience-id` 形式で指定し、作成前に本文とevidenceを読みます。
schema条件を満たすだけでは作成しません。候補の原則、適用scope、限界、反例、confidence、
全source IDを利用者へ提示し、明示承認を得た後だけ新規作成または昇格します。
...
- `related_experiences`と`supersedes`には同じprojectの実在IDだけを指定し、自己参照や
  両方への同一ID指定を行いません。
- memoryは現在の再利用可能な原則として更新できますが、source experience、条件、限界を保持します。
- 後継memoryが原則を置き換える場合は、後継をactiveで先に保存し、旧memoryを
  `status: superseded`、`superseded_by: <後継ID>`へrevision付き更新します。
- 誤りなどで後継を設けず利用を止めるmemoryは`status: retracted`にします。通常検索はactiveだけを
  返すため、廃止記録を監査するときはstatusを明示します。
- 文書削除は通常の整理手段にしません。重複や誤記でも、参照元とGit履歴を確認し、利用者の
  明示承認を得てから実施します。delete tool/commandをまずconfirmなしでdry-runし、revision一致、
  参照ゼロ、validation errorゼロを確認した後だけ同じrevisionへconfirmを付けます。
...
secret、credential、access token、PII（個人情報）、顧客dataをtitle、body、metadata、evidence、logへ
保存しません。logやcommand outputを根拠にするときは、機密値をredactしてから安全な抜粋または
参照だけを記録します。安全にredactできない場合は、内容を複製せず、非機密の説明と
「根拠をvaultへ保持できない」という限界だけを記録します。

validationや保守の報告でも、検出した値そのものを表示しません。影響するpathと種類だけを報告し、
修復には存在する`refinery-project`、`refinery-experience`、`refinery-memory`、
`refinery-maintenance` Skillと文書化されたcommandだけを使います。存在しないrepair skillや
commandを案内しません。
...
- [ ] evidenceを実際に確認したか。
- [ ] secret、credential、token、個人情報、顧客data、未redact logを含まないか。
